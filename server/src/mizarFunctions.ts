    ...
    numberOfErrors:number)
{
    ...
    // エラーがあれば、その数を出力
    if (numberOfErrors){
        process.stdout.write(" *" + numberOfErrors);
    }
...
):Promise<string>
{
    ...
    const result:Promise<string> = new Promise((resolve) => {
        makeenvProcess.on('close', () => {
            ...
            process.stdout.write("Running " + path.basename(command)
                                + " on " + fileName + '\n\n');
            ...
            carrier.carry(commandProcess.stdout, (line:string) => {
                ...
                if (phases.indexOf(phase) === -1){
                    if (phases.length !== 0){
                        ...
                        addMissingHashTags(numberOfProgress, numberOfErrors);
                    }
                    // 出力の項目を横並びにするために，スペースを補完する
                    process.stdout.write(padSpace(phase) +':');
